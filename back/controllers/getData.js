...
exports.getTravelData = async (req, res) => {
  try {
    ...
    const result = await database.query(
      'SELECT project_idx, project_title, start_date, end_date, planner_title, planner_description, planner_date FROM travel_project WHERE user_idx=$1 ORDER BY project_date DESC LIMIT 1',
      [user_idx]
    );
...
exports.getProjectData = async (req, res) => {
  ...
  try {
    const result = await database.query(
      'SELECT project_idx, project_title, start_date, end_date, planner_title, planner_description, planner_date  FROM travel_project WHERE user_idx=$1 ORDER BY planner_date DESC LIMIT 8',
      [user_idx]
    );
